...
struct SBPWM: GPIO {
    ...
    private static let PWM_PERIOD = 100
     ...
     */
    func initPin() -> Bool {
        writeStringToFile(SBPWM.PWM_STATE, path: getStatePath())
        if let val = PWMOCP[id], exportPath = getExportPath(), configPath = getConfigPath() {
            let exportSuccess = writeStringToFile(String(val.num), path: exportPath)
            let periodSuccess = writeStringToFile(String(SBPWM.PWM_PERIOD), path: configPath + "/" + SBPWM.PWM_PERIOD_FILE_NAME)
    ...
    func isPinActive() -> Bool {
        if let value = readStringFromFile(getStatePath()) {
    ...
    */
    ...
    private func getOcpPath() -> String? {
        ...
        if let val = PWMOCP[id] {
            ...
            var num = 0
            while dir != nil {
                let listing = readdir(dir)
                let file = listing.memory.d_name
                if (file.0 != 46) {
                    num = file.7 - 48
                    break
                }
            ...
            self.ocpPath = SBPWM.PWM_BASE_OCP_PATH + val.dir + mDirName + "/"
            return ocpPath
        }
        return nil
   ...
   private func getConfigPath() -> String? {
    ...
    /**
     ...
     - Parameter newValue:  The value for the duty cycle.  This can be from 0 -> 100
    ...
    func setValue(newValue: Int) -> Bool {
        ...
        if let configPath = getConfigPath() {
            if !writeStringToFile(String(newValue), path: configPath + "/" + SBPWM.PWM_DUTY_CYCLE_FILE_NAME) {
                return false
            }
            return true
        }
    ...
    */
    func setEnable(enable: Bool) -> Bool {
        ...
        if let configPath = getConfigPath() {
            if !writeStringToFile(value, path: configPath + "/" + SBPWM.PWM_ENABLE_FILE_NAME) {
                return false
            }
            return true
        }
